...
class Trainer(object):
    ...
    def update_train_state(self):
        ...
        print ("[EPOCH]: {0} | [LR]: {1} | [TRAIN SENS]: {1:.2f}% | [TRAIN SPEC]: {2:.2f}% | [TRAIN PREC]: {3:.2f}% | [TRAIN F1]: {4:.2f}%".format(
          self.train_state['epoch_index'], self.train_state['learning_rate'], 
            self.train_state['train_sens'][-1], self.train_state['train_spec'][-1], 
            self.train_state['train_prec'][-1], self.train_state['train_f1'][-1]))
 
        print ("[EPOCH]: {0} | [LR]: {1} | [VAL SENS]: {1:.2f}% | [VAL SPEC]: {2:.2f}% | [VAL PREC]: {3:.2f}% | [VAL F1]: {4:.2f}%".format(
          self.train_state['epoch_index'], self.train_state['learning_rate'], 
    ...
    def collate_fn(self, batch):
        ...
        processed_batch["paper"] = torch.LongTensor(
            processed_batch["paper"])
        processed_batch["label"] = torch.LongTensor(
...
print("Test loss: {0:.3f}".format(trainer.train_state['test_loss']))
print("Test accuracy: {0:.2f}%".format(trainer.train_state['test_acc']))
print("Test sensitivity: {0:.2f}".format(trainer.train_state['test_sens']))
print("Test specificity: {0:.2f}%".format(trainer.train_state['test_spec']))
print("Test precision: {0:.2f}%".format(trainer.train_state['test_prec']))
print("Test f1: {0:.2f}%".format(trainer.train_state['test_f1']))
