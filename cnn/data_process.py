...
args = Namespace(
    ...
    save_dir=path+'codes/cnn_20190626',
    train_size=0.80,
    val_size=0.10,
    test_size=0.10,
...
#len(split_df[split_df['label_random']==1])
#len(split_df[(split_df['split']=='train') & (split_df['label_random']==1)])
#len(split_df[(split_df['split']=='val') & (split_df['label_random']==1)])
#len(split_df[(split_df['split']=='test') & (split_df['label_random']==1)])
#
#len(split_df[split_df['label_blind']==1])
#len(split_df[(split_df['split']=='train') & (split_df['label_blind']==1)])
#len(split_df[(split_df['split']=='val') & (split_df['label_blind']==1)])
#len(split_df[(split_df['split']=='test') & (split_df['label_blind']==1)])
...
class PapersDataset(Dataset):
    def __init__(self, df, vectorizer):
        ...
        self.min_seq_length = min(map(get_length, df.text)) + 2 # (<BEGIN> + <END>)
        self.ave_seq_length = mean(map(get_length, df.text)) + 2 # (<BEGIN> + <END>)
        self.seq_length = map(get_length, df.text)
...
#print(dataset.max_seq_length)  # 611005
#print(dataset.min_seq_length)  # 1123        
#print(dataset.ave_seq_length)  # 8466
#
...
#    l.append(len(dataset[i]['paper']))
#seqlen = pd.DataFrame({'seqlen':l})
#seqlen.to_csv('seqlen.csv')


#import matplotlib.pyplot as plt
#plt.hist(np.clip(l, 0, 20000), bins=50)
