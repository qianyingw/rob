...
#              }
    ...
    pipeline = Pipeline([
        ...
        ('sgd', SGDClassifier(random_state=66)),
    ])   
         
#    parameters = {  'vect__min_df': (2,4,8,16,32,64),
#                    'vect__ngram_range': ((1,1), (1,2)),   
#                    'vect__max_features': (1000,2000,3000,4000,5000,6000,7000,8000,9000,10000),
...
#                 }
    ...
    clf = GridSearchCV(pipeline, parameters, cv=3, verbose=1, scoring=scorer, refit='F1', return_train_score=False)  
#    clf = GridSearchCV(pipeline, parameters, cv=3, n_jobs=2, pre_dispatch=2*2, verbose=1, scoring=scorer, refit='F1', return_train_score=False)      
